...
const AppliedJobs = () => {
  ...
  return (
    <div>
      <Header>
        <h1 className=" font-bold text-3xl my-32 pb-32 text-[#1A1919] text-center">
          Applied Jobs{" "}
        </h1>
      </Header>
      ...
      <div className="  mx-auto container text-right ">
        <div className="dropdown dropdown-bottom dropdown-end">
          <label
            tabIndex={0}
            className="btn bg-gray-200 m-1 border-none text-[#474747] hover:bg-gray-400"
          >
            Filter by <ChevronDownIcon className="ml-4  h-6 w-6" />
          </label>
          <ul
            tabIndex={0}
          ...
          >
            <li>
              <button onClick={handleSortOnsite}>Onsite job</button>
            </li>
            <li>
              <button onClick={handleSortRemote}>Remote job</button>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div className="mx-auto container">
        {displayData.map((job) => (
          <Applied job={job} key={job.id}></Applied>
        ))}
      </div>
